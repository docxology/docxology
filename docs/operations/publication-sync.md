...
**Apply does not refresh every gated surface.** It deliberately stops at the publication chain. After a successful apply you must still run the surfaces in [Regenerate Dependent Surfaces](#regenerate-dependent-surfaces) below — resume/CV exports, current counts, agent data (`claims.json`), the `exports`/`updates`/`evidence`/`reconciliation` pages, and the live-site verification snapshot are **not** regenerated by apply and will fail `validate_repo` until rebuilt. Apply also does **not** update the hand-maintained `**N** indexed paper folders` prose count in [`pages/BIBLIOGRAPHY.md`](../../pages/BIBLIOGRAPHY.md); when a `create_new` action adds a new folder, bump that number by hand (see the regeneration section).
...
There is no single "rebuild everything" orchestrator. `validate_repo.py` runs every
generator with `--check` in its authoritative order and fails on the first stale output;
the chain below regenerates, **in dependency order**, the surfaces that apply does not
touch, so a single pass reaches a green validation instead of whack-a-mole.

Run this complete sequence after any apply or manual curation. Each output feeds the
next, so do not reorder:

```bash
# 1. Counts first — agent data, resume, and prose totals all read from these.
uv run python3 code/orchestrators/build_current_counts.py
uv run python3 code/orchestrators/export_agent_data.py        # refreshes data/claims.json from counts
uv run python3 code/orchestrators/build_resume.py --all       # resume reads claims.json + counts

# 2. Static pages that encode work/paper totals.
uv run python3 code/orchestrators/build_exports_page.py
uv run python3 code/orchestrators/build_updates_page.py
uv run python3 code/orchestrators/build_evidence_page.py
uv run python3 code/orchestrators/build_reconciliation_report.py

# 3. Indexes derived from the above (catalog -> search index -> feed).
uv run python3 code/orchestrators/build_catalog.py
uv run python3 code/orchestrators/build_search_index.py
uv run python3 code/orchestrators/generate_feed.py
uv run python3 code/orchestrators/audit_assets.py

# 4. Hand-fix the prose paper-folder count if a create_new added a new folder
#    (apply bumps the works total but NOT the "**N** indexed paper folders" line):
#    edit pages/BIBLIOGRAPHY.md so "**N** indexed paper folders" equals the new
#    folder count, then continue.

# 5. Refresh the live-site verification snapshot so its expected_counts match local
...
# 6. Manifest LAST — it hashes every generated file, so it must run after all of them.
...
If `validate_repo.py` still reports `Stale generated <file>`, run the matching generator
from [`GENERATED.md`](../../GENERATED.md) and re-run the manifest + validation. Regenerate
[`sitemap.xml`](../../sitemap.xml) **after committing** (its `<lastmod>` is derived from git
