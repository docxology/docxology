...
## MIN — Agent-navigation doc fixes (2026-08-31 pass)

### MIN-01 — GENERATED.md missing `llms.txt` output row

- Priority: Minor (resolved 2026-08-31)
- Files: `GENERATED.md`, `code/orchestrators/sync_site_facts.py`
- `sync_site_facts.py` rewrites the volatile count sentence in `llms.txt`, but
  the GENERATED.md "Volatile site facts" row did not list `llms.txt` as an
  output. Fixed by adding it; no code change needed.

### MIN-02 — Stale dated report pointer in AGENT_START.md

- Priority: Minor (resolved 2026-08-31)
- Files: `AGENT_START.md`
- External-link triage recipe pointed at a hard-coded 2026-05-15 report while
  the current triage is dated 2026-08-29 and regenerated by
  `build_external_link_triage.py`. Fixed by pointing at the GENERATED.md row
  and the two-command refresh path.
