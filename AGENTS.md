...
- After Google Scholar metrics change, record a direct authenticated observation, update `data/scholar-snapshot.json` and the SHA-256-bound `data/scholar-verification-receipt.json`, run `code/orchestrators/sync_scholar_metrics.py`, and regenerate claim/resume/public outputs; keep hand-authored docs pointed at the snapshot instead of repeating the current count.
- Homepage teaching blurbs: BIOL-1 General Biology — College of the Redwoods, Pelican Bay, Spring and Fall 2026; BIOL-8 — Human Biology, College of the Redwoods, Spring 2026.
- AII Textbook Group site copy: nine completed 2022-textbook cohorts plus a live first 2026 Fundamentals cohort; link the Parr/Pezzulo/Friston MIT Press OA monograph and the Namjoshi Fundamentals monograph as in the Educator line.
...
- **Google Scholar** single source of truth: `data/scholar-snapshot.json`, propagated by `code/orchestrators/sync_scholar_metrics.py` (idempotent; `--check` exits 1 on drift). `data/scholar-verification-receipt.json` must bind its exact SHA-256 to a direct authenticated observation; the check fails closed if it is absent, stale, anonymous, or mismatched. `export_agent_data.py` reads the snapshot for claims. Publish only after a **direct** (non-anonymous/non-cached UI) Scholar verify: update snapshot (`as_of`, `method`, append to `history`) and its receipt, run the sync orchestrator, regenerate `data/claims.json` and the evidence page; never publish a citation count above the latest direct-fetch value. Public metrics use profile `DXjPFtYAAAAJ`; ORCID also links `Y2bMf3MAAAAJ`—consolidate to avoid split graphs.
...
- Independent anchors: AII **EIN 88-2985125** (see [ProPublica Nonprofit Explorer](https://projects.propublica.org/nonprofits/organizations/882985125)); NSF PRFB **DBI-2010290** ([official award record](https://api.nsf.gov/services/v1/awards/2010290.json), 2020-10-01 to 2023-09-30). `pages/VERIFICATION_LOG.md` + `data/verification-log.json` record dated evidence passes and should stay paired when updated.
- College of the Redwoods teaching (BIOL-1 at Pelican Bay in Spring and Fall 2026; BIOL-8 Human Biology in Spring 2026) is **principal-confirmed instructor-of-record**; do not remove or soften because a public WebAdvisor schedule may not yet list it.
