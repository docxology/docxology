...
def _write_paper_fixture(
    ...
    *,
...
) -> tuple[Path, Path, rd.DocumentPaths]:
    """Create a real minimal paper tree with explicit generator inputs."""
    ...
    return papers, folder, rd.DocumentPaths(papers, bibliography_path)
...
def test_generated_documents_are_deterministic_and_check_detects_real_drift(tmp_path: Path):
    papers, folder, paths = _write_paper_fixture(tmp_path)
    legacy_log = papers / "regenerate_docs.log"

    assert rd.main(["--apply"], paths=paths) == 0
    ...
    assert rd.main(["--check"], paths=paths) == 0
    ...
    assert rd.main(["--check"], paths=paths) == 1
    assert managed_path.read_text(encoding="utf-8").endswith("<!-- manual drift -->\n")
    assert manifest_path.read_text(encoding="utf-8") == manifest_before
    assert not legacy_log.exists()


def test_check_rejects_semantically_valid_but_noncanonical_manifest_bytes(tmp_path: Path):
    papers, _folder, paths = _write_paper_fixture(tmp_path)
...
def test_symlinked_manifest_is_rejected_before_any_read_or_write(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="symlinked generated-document manifest"):
        rd.load_generated_documents_manifest(manifest_path, paths=paths)
    assert rd.main(["--check"], paths=paths) == 2
    assert rd.main(["--apply"], paths=paths) == 2
    assert external_manifest.read_text(encoding="utf-8") == external_content


def test_manifest_path_outside_papers_is_rejected_before_reading(tmp_path: Path):
    """The optional manifest argument cannot widen the renderer's ownership root."""
    _papers, _folder, paths = _write_paper_fixture(tmp_path)
    external_manifest = tmp_path / "external-manifest.json"
    external_manifest.write_text('{"version": 1, "documents": []}\n', encoding="utf-8")

    with pytest.raises(ValueError, match="must be contained"):
        rd.load_generated_documents_manifest(external_manifest, paths=paths)
    assert rd.main(["--check", "--manifest", str(external_manifest)], paths=paths) == 2


def test_symlinked_paper_folder_is_rejected_before_rendering(tmp_path: Path):
    """A folder link cannot make rendering create documents outside papers/."""
    _papers, folder, paths = _write_paper_fixture(tmp_path)
    ...
    with pytest.raises(ValueError, match="symlinked paper folder"):
        rd.paper_folders(paths=paths)
    assert rd.main(["--check"], paths=paths) == 2
    assert rd.main(["--apply"], paths=paths) == 2
    assert not any((external_folder / filename).exists() for filename in rd.DOCUMENT_FILENAMES)


def test_symlinked_papers_root_is_rejected_before_metadata_loading(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="symlinked papers root"):
        rd.load_metadata(paths=paths)
    assert rd.main(["--check"], paths=paths) == 2
    assert rd.main(["--apply"], paths=paths) == 2


def test_symlinked_folder_metadata_is_rejected_before_reading(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="symlinked folder metadata"):
        rd.resolved_metadata("2026_Example", rd.load_metadata(paths=paths), paths=paths)
    assert rd.main(["--check"], paths=paths) == 2
    assert rd.main(["--apply"], paths=paths) == 2


def test_symlinked_manifest_owned_target_fails_plan_check_and_apply(tmp_path: Path):
    ...
    metadata = rd.load_metadata(paths=paths)
    bibliography = rd.parse_bibliography(paths=paths)
    folders = rd.paper_folders(paths=paths)
    manifest_entries = rd.load_generated_documents_manifest(paths=paths)
    metadata_by_folder = {
        name: rd.resolved_metadata(name, metadata, paths=paths)
        for name in folders
    }
...
def test_hard_linked_manifest_owned_target_cannot_overwrite_external_alias(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="hard-linked managed document"):
        rd.plan_document_changes(
            ["2026_Example"],
            {
                "2026_Example": rd.resolved_metadata(
                    "2026_Example",
                    rd.load_metadata(paths=paths),
                    paths=paths,
                )
            },
            rd.parse_bibliography(paths=paths),
            rd.load_generated_documents_manifest(paths=paths),
            adopt_existing=False,
            paths=paths,
        )
    assert rd.main(["--check"], paths=paths) == 2
...
def test_hard_linked_manifest_cannot_overwrite_external_alias(tmp_path: Path):
...
def test_unlisted_hand_authored_document_is_never_overwritten(tmp_path: Path):
    papers, folder, paths = _write_paper_fixture(tmp_path)
    ...
    assert rd.main(["--apply", "--force"], paths=paths) == 0
...
def test_explicit_adoption_requires_manifest_opt_in(tmp_path: Path):
    papers, folder, paths = _write_paper_fixture(tmp_path)
    readme_path = folder / "README.md"
    ...
    assert rd.main(["--apply"], paths=paths) == 1
    assert readme_path.read_text(encoding="utf-8") == original
    assert rd.main(["--adopt-existing", "--apply"], paths=paths) == 0
...
def test_doi_audit_treats_artifact_doi_as_informational_only(
    ...
    _papers, _folder, paths = _write_paper_fixture(
        tmp_path,
        metadata_doi=ARTIFACT_DOI,
        artifact_doi=CANONICAL_DOI,
    )

    assert rd.main(["--doi-audit"], paths=paths) == 1
...
def test_bibliography_doi_is_rendered_as_canonical_while_artifact_is_labeled(tmp_path: Path):
    _papers, _folder, paths = _write_paper_fixture(
        tmp_path,
    ...
    bibliography = rd.parse_bibliography(paths=paths)
    meta = rd.resolved_metadata("2026_Example", rd.load_metadata(paths=paths), paths=paths)

    readme = rd.generate_readme(
        "2026_Example",
        meta,
        bibliography["2026_Example"],
        paths=paths,
    )
