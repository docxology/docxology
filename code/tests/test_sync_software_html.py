...
def test_dry_run_cli():
    ...
    assert "OK dry-run: 50 docxology + 32 AII rows" in result.stdout
...
def test_collection_page_entity_count():
    ...
    assert len(collection["mainEntity"]) == 82
    assert "50 original repositories" in collection["description"]
