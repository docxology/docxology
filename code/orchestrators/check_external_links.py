...
from docxology_tools.report_paths import dated_report_path, generated_timestamp, latest_report, source_commit, source_worktree_state  # noqa: E402
...
IGNORE_PREFIXES = (
    ...
    # Footer build-stamp permalinks. Every regeneration rewrites the stamp,
    # so keeping these in the cached coverage contract made the gate fail on
    # the commit after each refresh — the report was always "missing" the
    # new SHA and carrying three "unexpected" old ones. They are also
    # unverifiable before the commit is pushed. Same forgiveness the page
    # generators already apply to the stamp itself
    # (build_stamp.reuse_on_disk_stamp) and build_sitemap applies to lastmod.
