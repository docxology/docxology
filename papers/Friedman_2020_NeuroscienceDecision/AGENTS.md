# AGENTS.md — The neuroscience of decision making (interview with Timothy Hanks)

**Paper**: The neuroscience of decision making (interview with Timothy Hanks) (2020)
**Area**: Genetics & Biomedical Research
**Authors**: Timothy Hanks, Alexandra Mikhailova, Daniel A. Friedman

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of The neuroscience of decision making (interview with Timothy Hanks)
**Tasks**:
- Analyze core argument: In this interview, Professor Tim Hanks discusses topics related to neuroscience, decision making, philosophy, and science as a career. Hanks explores ...
- Evaluate: Provides Tim Hanks's perspective on computational decision-making neuroscience
- Evaluate: Discusses connections between neural decision-making, free will, and mental health
- Evaluate: Highlights integrative approaches to studying brain decision processes

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Scholarly interview
- Assess and document: Computational neuroscience discussion
- Assess and document: Decision-making framework analysis

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this genetics & biomedical research work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: decision making, computational neuroscience, Bayesian brain, free will, mental health
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

## Extraction Log

- **Source PDF**: `Friedman_2020_NeuroscienceDecision.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
