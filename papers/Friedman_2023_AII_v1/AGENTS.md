# AGENTS.md — The Active Inference Institute and Active Inference Ecosystem (v1)

**Paper**: The Active Inference Institute and Active Inference Ecosystem (v1) (2023)
**Area**: Active Inference
**Authors**: Active Inference Institute

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of The Active Inference Institute and Active Inference Ecosystem (v1)
**Tasks**:
- Analyze core argument: An overview of the Active Inference Institute (AII), describing its mission, organizational structure, projects, and community. The institute serves a...
- Evaluate: Documents the Active Inference Institute mission and structure
- Evaluate: Describes open-science organizational model for Active Inference research

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Organizational documentation
- Assess and document: Open science methodology

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this active inference work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: Active Inference Institute, open science, research organization, Free Energy Principle, community building
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

## Extraction Log

- **Source PDF**: `Friedman_2023_AII_v1.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
