# AGENTS.md — Four-fold Fields of Quantum Dreams

**Paper**: Four-fold Fields of Quantum Dreams (2024)
**Area**: Art
**Authors**: Daniel Ari Friedman, Dean Tickles

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of Four-fold Fields of Quantum Dreams
**Tasks**:
- Analyze core argument: Four-fold Fields of Quantum Dreams explores the intersections of quantum mechanics, art, phenomenology, and Active Inference through a structured visu...
- Evaluate: Maps between quantum mechanical concepts and artistic-experiential domains
- Evaluate: Develops four-fold framework bridging physics, art, and phenomenology

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Visual-phenomenological analysis
- Assess and document: Quantum-artistic comparative mapping
- Assess and document: Structured symbolic analysis

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this art work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: quantum mechanics, art, phenomenology, Active Inference, visual analysis
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

### 💼 Consultant Agent
**Focus**: Practical application and knowledge transfer
**Tasks**:
- Evaluates practical applications and implementation strategies for this research
- Maps findings to industry challenges, Active Inference, and technological solutions
- Facilitates knowledge transfer to external teams and collaborators

## Extraction Log

- **Source PDF**: `Friedman_2024_QuantumDreams.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
