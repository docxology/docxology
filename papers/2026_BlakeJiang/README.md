# 📄 The Architecture of False Gods: William Blake, Professor Jiang, and the Active Inference Corrective to Single Vision

**Daniel Ari Friedman** (2026) · *Zenodo*

[![DOI](https://img.shields.io/badge/DOI-10.5281%2Fzenodo.20144983](https:%2F%2Fdoi.org%2F10.5281%2Fzenodo.20144983)-blue)]([10.5281/zenodo.20144983](https://doi.org/10.5281/zenodo.20144983))

---

## Abstract

> This rapid-publication essay synthesizes three intellectual frameworks examining cognitive closure. The author analyzes Professor Jiang Xueqin's commentary on artificial intelligence through William Blake's late-18th-century perceptual analysis and Active Inference terminology. The core convergence identified is closure of the perceiving system around its own top-down expectations, which Blake ter...

## Keywords

`William Blake` · `Active Inference` · `Artificial Intelligence` · `Cognitive Science` · `Free Energy Principle` · `AI Architecture` · `Consciousness Capture` · `Generative Models` · `Newton's Sleep` · `Single Vision`

## Methods

- Visual and symbolic analysis
- Cross-domain pattern mapping

## Key Findings

- Essay mapping Jiang Xueqin's artificial-intelligence critique through William Blake's theory of single vision and Active Inference concepts including pathological prior dominance.

## Artifacts

- DOI: [10.5281/zenodo.20144983](https://doi.org/10.5281/zenodo.20144983)
- PDF SHA-256: See zenodo_record

## Citation

> Daniel Ari Friedman (2026). *The Architecture of False Gods: William Blake, Professor Jiang, and the Active Inference Corrective to Single Vision*. Zenodo.

## Related

- [Full Bibliography](../../pages/BIBLIOGRAPHY.md)
- [All Papers](../README.md)