# AGENTS.md — Emergent Teams for Complex Threats

**Paper**: Emergent Teams for Complex Threats (2020)
**Area**: Cognitive Security
**Authors**: Daniel A. Friedman, R.J. Cordes, Scott David

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of Emergent Teams for Complex Threats
**Tasks**:
- Analyze core argument: Innovation teams formed in incubators, research accelerators, and hackathons need to quickly align on narrative, workflow, and objectives. This paper ...
- Evaluate: Introduces the Facilitator's Catechism for rapid team alignment
- Evaluate: Adapts military operations orders for civilian innovation contexts
- Evaluate: Provides structured protocols for emergent team formation

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Operations order framework development
- Assess and document: Military-to-civilian methodology transfer
- Assess and document: Team process design
- Assess and document: Organizational behavior analysis

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this cognitive security work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: emergent teams, operations orders, team formation, innovation, facilitation
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

### 💼 Consultant Agent
**Focus**: Practical application and knowledge transfer
**Tasks**:
- Evaluates practical applications and implementation strategies for this research
- Maps findings to industry challenges, Active Inference, and technological solutions
- Facilitates knowledge transfer to external teams and collaborators

## Extraction Log

- **Source PDF**: `Friedman_2020_EmergentTeams.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
