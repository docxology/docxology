<!-- docxology:generated-document README.md; ownership=explicit-manifest -->

# 🎨 AgentAndNiche

**Daniel Ari Friedman** (2025) · *Zenodo*

[![DOI](https://img.shields.io/badge/DOI-10.5281%2Fzenodo.17235137-blue)](https://doi.org/10.5281/zenodo.17235137)

---

## Abstract

> Synthesis of Agent and Niche is a visionary art-philosophy dialogue between William Blake's The Marriage of Heaven and Hell and ecological psychology, mediated by Active Inference and Buckminster Fuller's Synergetics. The text juxtaposes Blake's prophetic verses with speculative ecological-cognitive commentary, exploring themes of contraries as generative principles, threshold dynamics...

## Keywords

`William Blake` · `Marriage of Heaven and Hell` · `ecological psychology` · `Active Inference` · `Synergetics` · `contraries` · `predictive processing` · `Markov blankets` · `visionary epistemology` · `art-science synthesis`

## Methods

- Visual and symbolic analysis
- Cross-domain pattern mapping

## Key Findings

- Synthesis of Agent and Niche is a visionary art-philosophy dialogue between William Blake's The Marriage of Heaven and Hell and ecological psychology, mediated by Active Inference and Buckminster Full

## Artifacts

- DOI: [10.5281/zenodo.17235137](https://doi.org/10.5281/zenodo.17235137)
- Zenodo record: [https://zenodo.org/records/17235137](https://zenodo.org/records/17235137)
- PDF: [2025_AgentAndNiche.pdf](2025_AgentAndNiche.pdf)
- PDF download: [SAN_DAF_10-01-2025.pdf](https://zenodo.org/api/records/17243488/files/SAN_DAF_10-01-2025.pdf/content)
- PDF SHA-256: [See Zenodo record](https://zenodo.org/records/17235137)

## Citation

> Daniel Ari Friedman (2025). *AgentAndNiche*. Zenodo. DOI: 10.5281/zenodo.17235137. URL: https://doi.org/10.5281/zenodo.17235137.

## Related

- [Full Bibliography](../../pages/BIBLIOGRAPHY.md)
- [All Papers](../README.md)
