# AGENTS.md — Increasing the Accessibility and Applicability of Active Inference

**Paper**: Increasing the Accessibility and Applicability of Active Inference (2025)
**Area**: Active Inference
**Authors**: Daniel A. Friedman, Active Inference Institute

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of Increasing the Accessibility and Applicability of Active Inference
**Tasks**:
- Analyze core argument: A Letter of Intent submitted to Dana Frontiers proposing to increase the accessibility and applicability of Active Inference through Generative Playbo...
- Evaluate: Proposes Generative Playbooks for accessible Active Inference learning
- Evaluate: Identifies barriers to Active Inference dissemination (technical, linguistic, cultural)
- Evaluate: Designs open-source summer school curriculum for different education levels

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Educational curriculum design
- Assess and document: Generative Playbook development
- Assess and document: Open-source educational materials
- Assess and document: Accessibility assessment across education levels

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this active inference work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: accessibility, Active Inference, pedagogy, summer school curriculum, Generative Playbooks
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

### 💼 Consultant Agent
**Focus**: Practical application and knowledge transfer
**Tasks**:
- Evaluates practical applications and implementation strategies for this research
- Maps findings to industry challenges, Active Inference, and technological solutions
- Facilitates knowledge transfer to external teams and collaborators

## Extraction Log

- **Source PDF**: `Friedman_2025_AccessibilityActiveInference.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
