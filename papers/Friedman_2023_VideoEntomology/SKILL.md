---
name: "VideoEntomology"
description: "We examine the transformative impact of video technology on experimental entomology. Video-based approaches enable high-throughput behavioral phenotyping, automated tracking of individuals in colonies..."
tags: ["entomology", "video-entomology", "behavioral-tracking", "automated-phenotyping", "insect-behavior", "experimental-methods"]
---

# Experimental Entomology in the Age of Video

**Daniel A. Friedman, Judith R. Wexler, Sebastian Alvarado** (2023) · Entomology

## Instructions

Use this skill when working with topics related to **video entomology, behavioral tracking, automated phenotyping, insect behavior**.

When applying this skill:
1. Apply video-based methods for insect behavioral tracking
1. Design high-throughput behavioral phenotyping experiments
1. Implement automated analysis pipelines for video entomology

## Key Concepts

- **video entomology**
- **behavioral tracking**
- **automated phenotyping**
- **insect behavior**
- **experimental methods**
- **high-throughput analysis**

## Methods & Techniques

- Video-based behavioral tracking
- Automated behavioral phenotyping
- Computer vision for entomology
- High-throughput behavioral analysis

## Key Findings

- Reviews video technology applications in experimental entomology
- Identifies best practices for video-based insect behavioral analysis

## Prerequisites

- Evolutionary biology and ecology fundamentals
- Social insect biology and colony organization

## Related Skills

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the complete publication catalog and related papers.
