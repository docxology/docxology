...
capped at 100 papers per keyword from deep_search.max_results_per_keyword in docs/manuscript/config
...
code_project (file docs/manuscript/references.bib);
...
Run snapshot. With the bundled docs/manuscript/config.yaml, the most recent pipeline execution evaluated
...
The project is configurable via a single docs/manuscript/config.yaml: changing the topic, year filters, backend
...
n + docs/manuscript/references.bib + output/enrichment_log.json. The orchestrator script then
...
aggregate_report.md, and a unified, deduplicated docs/manuscript/references_deep.bib with collision-
free citation keys.
...
4
Results
Run snapshot. With the bundled docs/manuscript/config.yaml the most recent execution evaluated the query
...
4.2
Output artefacts
After running scripts/run_search_pipeline.py against the default docs/manuscript/config.yaml, the project
produces:
...
• docs/manuscript/references.bib — the auto-populated bibliography from the single-query pipeline
(merged with any other docs/manuscript/*.bib at PDF render time).
...
make a single docs/manuscript/config.yaml the only artifact a reviewer needs.
...
manuscript source in docs/manuscript/. The Mermaid flowchart in this subsection (rendered in the HTML build;
...
deterministic build that can be re-run from scratch [Peng, 2011]. The bundled docs/manuscript/config.yaml is
...
BibTeX file is written under docs/manuscript/references_deep.bib so it can be cited from the manuscript
...
block from docs/manuscript/config.yaml, runs one
...
written to docs/manuscript/references_deep.bib, output/deep_search/aggregate.json, and output/deep
...
• docs/manuscript/references_deep.bib — auto-generated, deduplicated unified BibTeX file.
...
crossref). All references are stored in docs/manuscript/references_deep.bib and resolved by the combined-PDF
pipeline (Pandoc --natbib + BibTeX over all docs/manuscript/*.bib files).
9.1
convex optimization
...
BibTeX and merges every docs/manuscript/*.bib for citation resolution:
• docs/manuscript/references.bib — single-query pipeline output (scripts/run_search_pipeline.py).
• docs/manuscript/references_deep.bib — deduplicated multi-keyword deep-search output (scripts/ru
