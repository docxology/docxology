# AGENTS.md — The Doors of Perception are the Threshold of Prediction: Active Inference and William Blake's Theory of Seeing

**Paper**: The Doors of Perception are the Threshold of Prediction: Active Inference and William Blake's Theory of Seeing (2026)
**Area**: Art
**Authors**: Daniel Ari Friedman

---

## Agent Roles

### 📖 Research Agent
**Focus**: Deep analysis of The Doors of Perception are the Threshold of Prediction: Active Inference and William Blake's Theory of Seeing
**Tasks**:
- Analyze core argument: The Doors of Perception are the Threshold of Prediction explores eight concordances between William Blake's prophetic vision and the mathematics of Ac...
- Evaluate: Maps eight concordances between Blake's vision and Active Inference mathematics
- Evaluate: Develops Thematic Atlas connecting prophetic epistemology to predictive processing

### 🔬 Methods Agent
**Focus**: Methodological rigor and replication
**Tasks**:
- Assess and document: Thematic concordance mapping
- Assess and document: Art-mathematics synthesis
- Assess and document: Active Inference application to aesthetics

### 📚 Citation Agent
**Focus**: Citation context and scholarly impact
**Tasks**:
- Track citations and impact of this art work
- Map connections to related works in the Friedman corpus
- Identify citing papers and downstream applications

### 🔗 Synthesis Agent
**Focus**: Cross-paper and cross-domain connections
**Tasks**:
- Connect themes: William Blake, Active Inference, perception, prediction, Thematic Atlas
- Link to related works in the docxology corpus
- Identify synergies across Active Inference, entomology, and cognitive security

## Extraction Log

- **Source PDF**: `Friedman_2026_DoorsOfPerception.pdf`
- **PDF Status**: ✅ Available
- **Documentation Quality**: Hand-curated from direct PDF analysis

## Related Papers

See [BIBLIOGRAPHY.md](../../BIBLIOGRAPHY.md) for the full publication catalog.
